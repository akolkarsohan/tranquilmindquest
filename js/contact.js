...
function handleNewsletterSubmit(e) {
    ...
    const button = newsletterForm.querySelector('.newsletter-btn, button[type="submit"]');
        ...
        .then(data => {
            if (data.success) {
                ...
                if (button) {
                    ...
                    button.style.background = 'var(--success-green, #7EE787)';
                    setTimeout(() => {
...
function toggleMobileChat() {
    ...
    if (chatbotColumn) {
        ...
        // Also toggle mobile-open on chatbot section for CSS visibility
        if (chatbotSection) {
            chatbotSection.classList.toggle('mobile-open');
        }
